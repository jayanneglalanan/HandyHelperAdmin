...
export default function ReportList() {
  ...
  const filtered = reports.filter(r => {
    ...
    const matchesSearch = r.reporterName.toLowerCase().includes(search.toLowerCase()) ||
      ...
      <Card padding="none">
        ...
        <div className="p-3 sm:p-4">
          <ResponsiveTable
            ...
            columns={[
              {
                key: 'info', header: 'Report',
                render: (r) => (
                  <div>
                    <div className="flex items-center gap-2">
                      <span className="text-sm font-medium text-gray-900 dark:text-white">{r.reason}</span>
                      <Badge>{r.type}</Badge>
                    </div>
              ...
              { key: 'status', header: 'Status', render: (r) => <StatusBadge status={r.status} /> },
              {
                key: 'actions', header: '', render: () => (
                  <button className="p-2 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg dark:hover:bg-blue-900/20 min-w-[36px] min-h-[36px] flex items-center justify-center">
                    <Eye size={16} />
                  </button>
                ),
              },
            ...
            mobileCardRender={(r) => (
              <div>
                ...
                <div className="flex items-center justify-between mt-2">
                  <span className="text-xs text-gray-400">{r.reporterName} → {r.reportedUserName}</span>
                  <span className="text-xs text-gray-400">{formatDate(r.filedDate)}</span>
                </div>
              </div>
            )}
          />
