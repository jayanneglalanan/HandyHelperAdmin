...
export default function DisputeList() {
  ...
  const getStatus = (d: any) => disputeStatuses[d.id] || d.status;

  const handleReview = (id: string) => {
    setDisputeStatuses(prev => ({ ...prev, [id]: 'investigating' }));
    showToast('Dispute is now under review');
  };

  const handleResolve = (id: string) => {
    setDisputeStatuses(prev => ({ ...prev, [id]: 'resolved' }));
    showToast('Dispute resolved');
  };

  const tabs = [
    { id: 'pending', label: 'Pending', count: disputes.filter(d => getStatus(d) === 'pending').length },
    { id: 'investigating', label: 'In Review', count: disputes.filter(d => getStatus(d) === 'investigating').length },
    { id: 'resolved', label: 'Resolved', count: disputes.filter(d => getStatus(d) === 'resolved').length },
  ];

  const filtered = disputes.filter(d => {
    const matchesTab = getStatus(d) === activeTab;
    const matchesSearch = d.clientName.toLowerCase().includes(search.toLowerCase()) || d.memberName.toLowerCase().includes(search.toLowerCase()) || d.reason.toLowerCase().includes(search.toLowerCase());
      ...
      <Card padding="none">
        ...
        <div className="p-3 sm:p-4">
          <ResponsiveTable
            ...
            keyExtractor={(d) => d.id}
            columns={[
              { key: 'info', header: 'Dispute', render: (d) => (
                <div><span className="text-sm font-medium text-gray-900 dark:text-white">{d.reason}</span><p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5 line-clamp-1">{d.description}</p></div>
              )},
              ...
              { key: 'status', header: 'Status', render: (d) => <StatusBadge status={getStatus(d)} /> },
              { key: 'actions', header: '', render: (d) => (
                <div className="flex gap-1">
                  {getStatus(d) === 'pending' && <Button variant="outline" size="sm" onClick={() => handleReview(d.id)}>Review</Button>}
                  {getStatus(d) === 'investigating' && <Button variant="success" size="sm" onClick={() => handleResolve(d.id)}>Resolve</Button>}
                </div>
              )},
            ]}
            mobileCardRender={(d) => (
              <div>
                <div className="flex items-center gap-2 mb-1"><StatusBadge status={getStatus(d)} /></div>
